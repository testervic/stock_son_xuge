...
#直播和疑难结果需要的老师ID返回
def seek_list(need_num, source_list = []):

    #默认筛选数量
    seek_num = need_num
    
    if source_list == 0:
        print '原数据为空无筛选结果!!!'.decode('utf-8')
        return 0
    else:
        #确定筛选数量
        if need_num > len(source_list):
            seek_num = len(source_list)

        #随机返回指定数量的老师ID的list
        return random.sample(source_list, seek_num)
...
def zb_comment(member_key):
    ...
    #获取可供筛选的直播老师ID
    seek_seller_id_list = seek_seller_id(zb_num['seek_num'])

    #指定直播评论流程中使用的老师ID
    seller_id_list = seek_list(zb_num['comment_num'], seek_seller_id_list)
...
def zb_dyjh(member_key):
    ...
    #指定疑难解惑流程中使用的老师ID
    seller_id_list = seek_list(zb_num['yljh_num'], seek_seller_id_list)
